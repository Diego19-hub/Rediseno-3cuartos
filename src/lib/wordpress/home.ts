...
const fallback = (): HomeContent => ({
  services: homeProvisional.services.map((item, index) => ({ id: -(index + 1), slug: ["diseno-branding", "desarrollo-web", "marketing-digital"][index], ...item, description: item.summary, capabilities: [], cta: homeProvisional.globalCta, order: index + 1, seo: { title: "", description: "", noindex: true }, isProvisional: true })),
  caseStudy: { id: -1, slug: "caso-destacado-provisional", ...homeProvisional.caseStudy, content: "Estrategia provisional pendiente de validación.", metrics: homeProvisional.caseStudy.metrics.map((metric) => ({ ...metric })), objectives: [], serviceIds: [], results: [], gallery: [], cta: { label: "Ver caso", url: "/casos-de-exito/caso-destacado-provisional" }, seo: { title: "", description: "", noindex: true }, isProvisional: true },
  ...
  resources: homeProvisional.resources.map((title, index) => ({ id: -(index + 1), slug: `recurso-provisional-${index + 1}`, title, excerpt: "Contenido demostrativo pendiente de aprobación.", featuredExcerpt: "Contenido demostrativo pendiente de aprobación.", content: "", cta: { label: "Explorar recursos", url: `/recursos/recurso-provisional-${index + 1}` }, readingTime: 0, publishedAt: "", categoryIds: [] })),
